...
function braceletUplinkMessage(prefix, bytes) {
  ...
  for (var i = 0; i < length; ++i) {
    var tmp1 = i + 1;
    data[prefix + "dev" + tmp1 + "mac"] = `${bytes[pos + 1].toString(16).padStart(2, "0").toUpperCase()}${bytes[pos + 2].toString(16).padStart(2, "0").toUpperCase()}${bytes[
      pos + 3
    ]
      .toString(16)
      .padStart(2, "0")
      .toUpperCase()}${bytes[pos + 4].toString(16).padStart(2, "0").toUpperCase()}${bytes[pos + 5].toString(16).padStart(2, "0").toUpperCase()}${bytes[pos + 6]
      .toString(16)
      .padStart(2, "0")
      .toUpperCase()}`; //(((bytes[pos+1] & 0xff) << 40) | ((bytes[pos+2] & 0xff) << 32) | ((bytes[pos+3] & 0xff) << 24) | ((bytes[pos+4] & 0xff) << 16) | ((bytes[pos+5] & 0xff) << 8) | ((bytes[pos+6] & 0xff))).toString(16);
